...
class PembayaranController extends Controller
{
	...
	public function actionBelumLunas()
	{
		$dataProvider = new CActiveDataProvider('Kunjungan', array(
			'criteria' => array(
				'with' => 'pembayaran',
				'condition' => 'pembayaran.id IS NULL OR pembayaran.status != "lunas"',
			),
		));

		$this->render('belumLunas', array(
			'dataProvider' => $dataProvider,
		));
	}
	...
	{
		$kunjungan = Kunjungan::model()->findByPk($id);
		$model = new Pembayaran;
		$model->id_kunjungan = $id;
		$model->total = $kunjungan->getTotalTagihan();
		$model->tanggal_pembayaran = date('Y-m-d');

		if (isset($_POST['Pembayaran'])) {
			$model->attributes = $_POST['Pembayaran'];
			$model->status = ($model->dibayar >= $model->total) ? 'lunas' : 'belum';

			if ($model->save()) {
				$this->redirect(['belumLunas']);
			}
		}
	...
	{
		$model = $this->loadModel($id);
		$this->renderPartial('cetak', ['model' => $model]);
	}
}
